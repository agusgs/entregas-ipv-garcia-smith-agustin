...
func _handle_move_input():
	...
	if move_direction != 0:
		...
		body.offset.x = body_offset * move_direction
...
func play_animation(name:String, custom_speed:float = 1.0):
	if animation_player.has_animation(name):
		animation_player.play(name, -1, custom_speed)
	else:
		print("Missing animation %s" % name)
 
func recover_health():
	PlayerData.set_current_health(max_health)
