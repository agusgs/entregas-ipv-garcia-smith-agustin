...
func target_direction():
	return fire_position.global_position.direction_to(target.global_position)

func fire():
	if target != null:
		body.flip_h = target_direction().x < 0
		var proj_instance = projectile_scene.instance()
		if projectile_container == null:
			projectile_container = get_parent()
		proj_instance.initialize(projectile_container, fire_position.global_position, target_direction())
