...
func play_sound_efecf(stream: AudioStream):
	player_sfx.stream = stream
	player_sfx.play()


func _on_PlayerAnimation_animation_finished(anim_name):
	state_machine._on_animation_finished(anim_name) # Replace with function body.
